"use client";

import { useEffect, useState } from "react";
import Image from "next/image";
import FadeInSection from "@/app/components/home/FadeInSection";
import SectionDivider from "@/app/components/home/SectionDivider";

interface Podcast {
  ep_name: string;
  cover: string;
  episode_number: string;
  date: string;
  episode_description: string;
  spotifylink: string;
  youtubelink: string;
}

function PodcastCard({ podcast }: { podcast: Podcast }) {
  return (
    <div className="group flex items-center gap-4 p-4 rounded-2xl border border-white/15 bg-white/[0.06] hover:border-[rgba(255,45,92,0.45)] hover:bg-white/[0.10] transition duration-300">
      <Image
        src="/images/media/podcastphoto.png"
        alt="Podcast Cover"
        width={64}
        height={64}
        className="rounded-lg shrink-0"
        unoptimized
      />
      <div className="flex-1 min-w-0">
        <div className="text-xs uppercase tracking-[0.2em] text-white/60 mb-1">
          Episode {podcast.episode_number}
        </div>
        <h3 className="text-base md:text-lg text-white leading-tight mb-2 truncate">
          {podcast.ep_name}
        </h3>
        <div className="flex flex-wrap gap-3">
          {podcast.spotifylink && (
            <a
              href={podcast.spotifylink}
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs font-semibold text-[#1db954] hover:text-white hover:[text-shadow:0_0_10px_rgba(29,185,84,0.7)] transition"
            >
              Listen on Spotify →
            </a>
          )}
          {podcast.youtubelink && (
            <a
              href={podcast.youtubelink}
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs font-semibold text-[#ff2d5c] hover:text-white hover:[text-shadow:0_0_10px_rgba(255,45,92,0.9)] transition"
            >
              Watch on YouTube →
            </a>
          )}
        </div>
      </div>
    </div>
  );
}

export default function MediaPage() {
  const [podcasts, setPodcasts] = useState<Podcast[]>([]);

  useEffect(() => {
    fetch("/json/media.json")
      .then((res) => res.json())
      .then((data) => setPodcasts(data.podcasts));
  }, []);

  useEffect(() => {
    const script = document.createElement("script");
    script.src = "https://www.instagram.com/embed.js";
    script.async = true;
    document.body.appendChild(script);

    return () => {
      document.body.removeChild(script);
    };
  }, []);

  const smallPodcasts = podcasts.slice(1);

  return (
    <div className="bg-[#050508] text-white overflow-x-clip">
      {/* Hero */}
      <section className="min-h-[calc(100vh-76px)] flex items-center justify-center relative overflow-hidden text-center border-b border-white/10 px-4 py-24">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(186,12,47,0.18),transparent_60%)] pointer-events-none" />
        <div className="relative z-10 max-w-4xl">
          <p className="text-sm md:text-base uppercase tracking-[0.3em] text-[#ff2d5c] [text-shadow:0_0_10px_rgba(255,45,92,0.7)] mb-4 animate-in fade-in slide-in-from-top-4 duration-1000 fill-mode-both">
            Stay Connected
          </p>
          <h1 className="text-5xl md:text-7xl mb-6 leading-tight animate-in fade-in slide-in-from-top-8 duration-1000 fill-mode-both">
            Media
          </h1>
          <p className="text-lg md:text-xl text-white/80 leading-8 max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300 fill-mode-both">
            The Clean Room podcast, social posts, and video content from the
            UGA Small Satellite Research Laboratory.
          </p>
        </div>
      </section>

      <SectionDivider />

      {/* Two columns: podcast on left, social on right */}
      <FadeInSection>
        <section className="py-20 px-4">
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12">
            {/* Left: The Clean Room */}
            <div>
              <p className="text-sm uppercase tracking-[0.25em] text-[#ff2d5c] [text-shadow:0_0_10px_rgba(255,45,92,0.6)] mb-3">
                The Podcast
              </p>
              <h2 className="text-3xl md:text-4xl mb-4">The Clean Room</h2>
              <p className="text-white/85 leading-7 mb-6">
                The University of Georgia&apos;s Small Satellite Research
                Laboratory presents a podcast all about spacecraft development.
              </p>

              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 mb-6">
                <iframe
                  className="w-full"
                  style={{ borderRadius: "12px" }}
                  src="https://open.spotify.com/embed/show/2NDHIJpmlPSIWgoZgYN0ya?utm_source=generator"
                  height="352"
                  allowFullScreen
                  allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
                  loading="lazy"
                />
              </div>

              {smallPodcasts.length === 0 ? (
                <p className="text-white/60">Loading episodes…</p>
              ) : (
                <div className="flex flex-col gap-3">
                  {smallPodcasts.map((podcast) => (
                    <PodcastCard
                      key={podcast.episode_number}
                      podcast={podcast}
                    />
                  ))}
                </div>
              )}
            </div>

            {/* Right: Social & Video */}
            <div>
              <p className="text-sm uppercase tracking-[0.25em] text-[#ff2d5c] [text-shadow:0_0_10px_rgba(255,45,92,0.6)] mb-3">
                Other Media
              </p>
              <h2 className="text-3xl md:text-4xl mb-4">Follow Along</h2>
              <p className="text-white/85 leading-7 mb-6">
                Follow us on Instagram to stay up to date, and check out the
                YouTube channel for mission and lab footage.
              </p>

              {/* Instagram */}
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 mb-6">
                <blockquote
                  className="instagram-media"
                  data-instgrm-permalink="https://www.instagram.com/ugassrl/?utm_source=ig_embed&utm_campaign=loading"
                  data-instgrm-version="14"
                  style={{
                    background: "#fff",
                    border: 0,
                    borderRadius: "12px",
                    boxShadow:
                      "0 0 1px 0 rgba(0,0,0,0.5), 0 1px 10px 0 rgba(0,0,0,0.15)",
                    margin: "1px",
                    maxWidth: "600px",
                    minWidth: "326px",
                    padding: 0,
                    width: "calc(100% - 2px)",
                  }}
                >
                  <div style={{ padding: "16px" }}>
                    <a
                      href="https://www.instagram.com/ugassrl/?utm_source=ig_embed&utm_campaign=loading"
                      style={{
                        background: "#03000f",
                        lineHeight: 0,
                        padding: "0 0",
                        textAlign: "center" as const,
                        textDecoration: "none",
                        width: "100%",
                      }}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "row" as const,
                          alignItems: "center",
                        }}
                      >
                        <div
                          style={{
                            backgroundColor: "#03000f",
                            borderRadius: "50%",
                            flexGrow: 0,
                            height: "40px",
                            marginRight: "14px",
                            width: "40px",
                          }}
                        />
                        <div
                          style={{
                            display: "flex",
                            flexDirection: "column" as const,
                            flexGrow: 1,
                            justifyContent: "center",
                          }}
                        >
                          <div
                            style={{
                              backgroundColor: "#03000f",
                              borderRadius: "4px",
                              flexGrow: 0,
                              height: "14px",
                              marginBottom: "6px",
                              width: "100px",
                            }}
                          />
                          <div
                            style={{
                              backgroundColor: "#03000f",
                              borderRadius: "4px",
                              flexGrow: 0,
                              height: "14px",
                              width: "60px",
                            }}
                          />
                        </div>
                      </div>
                      <div style={{ padding: "19% 0" }} />
                      <div
                        style={{
                          display: "block",
                          height: "50px",
                          margin: "0 auto 12px",
                          width: "50px",
                        }}
                      >
                        <svg
                          width="50px"
                          height="50px"
                          viewBox="0 0 60 60"
                          version="1.1"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <g
                            stroke="none"
                            strokeWidth="1"
                            fill="none"
                            fillRule="evenodd"
                          >
                            <g
                              transform="translate(-511.000000, -20.000000)"
                              fill="#000000"
                            >
                              <g>
                                <path d="M556.869,30.41 C554.814,30.41 553.148,32.076 553.148,34.131 C553.148,36.186 554.814,37.852 556.869,37.852 C558.924,37.852 560.59,36.186 560.59,34.131 C560.59,32.076 558.924,30.41 556.869,30.41 M541,60.657 C535.114,60.657 530.342,55.887 530.342,50 C530.342,44.114 535.114,39.342 541,39.342 C546.887,39.342 551.658,44.114 551.658,50 C551.658,55.887 546.887,60.657 541,60.657 M541,33.886 C532.1,33.886 524.886,41.1 524.886,50 C524.886,58.899 532.1,66.113 541,66.113 C549.9,66.113 557.115,58.899 557.115,50 C557.115,41.1 549.9,33.886 541,33.886 M565.378,62.101 C565.244,65.022 564.756,66.606 564.346,67.663 C563.803,69.06 563.154,70.057 562.106,71.106 C561.058,72.155 560.06,72.803 558.662,73.347 C557.607,73.757 556.021,74.244 553.102,74.378 C549.944,74.521 548.997,74.552 541,74.552 C533.003,74.552 532.056,74.521 528.898,74.378 C525.979,74.244 524.393,73.757 523.338,73.347 C521.94,72.803 520.942,72.155 519.894,71.106 C518.846,70.057 518.197,69.06 517.654,67.663 C517.244,66.606 516.755,65.022 516.623,62.101 C516.479,58.943 516.448,57.996 516.448,50 C516.448,42.003 516.479,41.056 516.623,37.899 C516.755,34.978 517.244,33.391 517.654,32.338 C518.197,30.938 518.846,29.942 519.894,28.894 C520.942,27.846 521.94,27.196 523.338,26.654 C524.393,26.244 525.979,25.756 528.898,25.623 C532.057,25.479 533.004,25.448 541,25.448 C548.997,25.448 549.943,25.479 553.102,25.623 C556.021,25.756 557.607,26.244 558.662,26.654 C560.06,27.196 561.058,27.846 562.106,28.894 C563.154,29.942 563.803,30.938 564.346,32.338 C564.756,33.391 565.244,34.978 565.378,37.899 C565.522,41.056 565.552,42.003 565.552,50 C565.552,57.996 565.522,58.943 565.378,62.101 M570.82,37.631 C570.674,34.438 570.167,32.258 569.425,30.349 C568.659,28.377 567.633,26.702 565.965,25.035 C564.297,23.368 562.623,22.342 560.652,21.575 C558.743,20.834 556.562,20.326 553.369,20.18 C550.169,20.033 549.148,20 541,20 C532.853,20 531.831,20.033 528.631,20.18 C525.438,20.326 523.257,20.834 521.349,21.575 C519.376,22.342 517.703,23.368 516.035,25.035 C514.368,26.702 513.342,28.377 512.574,30.349 C511.834,32.258 511.326,34.438 511.181,37.631 C511.035,40.831 511,41.851 511,50 C511,58.147 511.035,59.17 511.181,62.369 C511.326,65.562 511.834,67.743 512.574,69.651 C513.342,71.625 514.368,73.296 516.035,74.965 C517.703,76.634 519.376,77.658 521.349,78.425 C523.257,79.167 525.438,79.673 528.631,79.82 C531.831,79.965 532.853,80.001 541,80.001 C549.148,80.001 550.169,79.965 553.369,79.82 C556.562,79.673 558.743,79.167 560.652,78.425 C562.623,77.658 564.297,76.634 565.965,74.965 C567.633,73.296 568.659,71.625 569.425,69.651 C570.167,67.743 570.674,65.562 570.82,62.369 C570.966,59.17 571,58.147 571,50 C571,41.851 570.966,40.831 570.82,37.631" />
                              </g>
                            </g>
                          </g>
                        </svg>
                      </div>
                      <div style={{ paddingTop: "8px" }}>
                        <div
                          style={{
                            color: "#3897f0",
                            fontSize: "14px",
                            fontStyle: "normal",
                            fontWeight: 550,
                            lineHeight: "18px",
                          }}
                        >
                          View this profile on Instagram
                        </div>
                      </div>
                      <div style={{ padding: "12.5% 0" }} />
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "row" as const,
                          marginBottom: "14px",
                          alignItems: "center",
                        }}
                      >
                        <div>
                          <div
                            style={{
                              backgroundColor: "#f4f4f4",
                              borderRadius: "50%",
                              height: "12.5px",
                              width: "12.5px",
                              transform: "translateX(0px) translateY(7px)",
                            }}
                          />
                          <div
                            style={{
                              backgroundColor: "#f4f4f4",
                              height: "12.5px",
                              transform:
                                "rotate(-45deg) translateX(3px) translateY(1px)",
                              width: "12.5px",
                              flexGrow: 0,
                              marginRight: "14px",
                              marginLeft: "2px",
                            }}
                          />
                          <div
                            style={{
                              backgroundColor: "#f4f4f4",
                              borderRadius: "50%",
                              height: "12.5px",
                              width: "12.5px",
                              transform: "translateX(9px) translateY(-18px)",
                            }}
                          />
                        </div>
                        <div style={{ marginLeft: "8px" }}>
                          <div
                            style={{
                              backgroundColor: "#f4f4f4",
                              borderRadius: "50%",
                              flexGrow: 0,
                              height: "20px",
                              width: "20px",
                            }}
                          />
                          <div
                            style={{
                              width: 0,
                              height: 0,
                              borderTop: "2px solid transparent",
                              borderLeft: "6px solid #f4f4f4",
                              borderBottom: "2px solid transparent",
                              transform:
                                "translateX(16px) translateY(-4px) rotate(30deg)",
                            }}
                          />
                        </div>
                        <div style={{ marginLeft: "auto" }}>
                          <div
                            style={{
                              width: "0px",
                              borderTop: "8px solid #f4f4f4",
                              borderRight: "8px solid transparent",
                              transform: "translateY(16px)",
                            }}
                          />
                          <div
                            style={{
                              backgroundColor: "#f4f4f4",
                              flexGrow: 0,
                              height: "12px",
                              width: "16px",
                              transform: "translateY(-4px)",
                            }}
                          />
                          <div
                            style={{
                              width: 0,
                              height: 0,
                              borderTop: "8px solid #f4f4f4",
                              borderLeft: "8px solid transparent",
                              transform: "translateY(-4px) translateX(8px)",
                            }}
                          />
                        </div>
                      </div>
                      <div
                        style={{
                          display: "flex",
                          flexDirection: "column" as const,
                          flexGrow: 1,
                          justifyContent: "center",
                          marginBottom: "24px",
                        }}
                      >
                        <div
                          style={{
                            backgroundColor: "#f4f4f4",
                            borderRadius: "4px",
                            flexGrow: 0,
                            height: "14px",
                            marginBottom: "6px",
                            width: "224px",
                          }}
                        />
                        <div
                          style={{
                            backgroundColor: "#f4f4f4",
                            borderRadius: "4px",
                            flexGrow: 0,
                            height: "14px",
                            width: "144px",
                          }}
                        />
                      </div>
                    </a>
                    <p
                      style={{
                        color: "#c9c8cd",
                        fontSize: "14px",
                        lineHeight: "17px",
                        marginBottom: 0,
                        marginTop: "8px",
                        overflow: "hidden",
                        padding: "8px 0 7px",
                        textAlign: "center" as const,
                        textOverflow: "ellipsis",
                        whiteSpace: "nowrap" as const,
                      }}
                    >
                      <a
                        href="https://www.instagram.com/ugassrl/?utm_source=ig_embed&utm_campaign=loading"
                        style={{
                          color: "#c9c8cd",
                          fontSize: "14px",
                          fontStyle: "normal",
                          fontWeight: "normal",
                          lineHeight: "17px",
                        }}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        UGA Small Sat Research Lab
                      </a>{" "}
                      (@
                      <a
                        href="https://www.instagram.com/ugassrl/?utm_source=ig_embed&utm_campaign=loading"
                        style={{
                          color: "#c9c8cd",
                          fontSize: "14px",
                          fontStyle: "normal",
                          fontWeight: "normal",
                          lineHeight: "17px",
                        }}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        ugassrl
                      </a>
                      ) &bull; Instagram photos and videos
                    </p>
                  </div>
                </blockquote>
              </div>

              {/* YouTube */}
              <a
                href="https://www.youtube.com/@ugassrl"
                target="_blank"
                rel="noopener noreferrer"
                className="group relative block rounded-2xl overflow-hidden border border-white/10 hover:border-[rgba(255,45,92,0.45)] transition"
              >
                <img
                  src="/images/media/ytscreenshot.png"
                  alt="UGA SSRL YouTube channel"
                  className="w-full block transition duration-500 group-hover:scale-[1.02] group-hover:brightness-[0.6]"
                />
                <div className="absolute inset-0 flex justify-center items-center opacity-0 group-hover:opacity-100 transition duration-300">
                  <span className="px-5 py-2 rounded-full bg-[#BA0C2F] text-white font-semibold shadow-[0_0_25px_rgba(186,12,47,0.7)]">
                    Follow us on YouTube
                  </span>
                </div>
              </a>
            </div>
          </div>
        </section>
      </FadeInSection>
    </div>
  );
}
