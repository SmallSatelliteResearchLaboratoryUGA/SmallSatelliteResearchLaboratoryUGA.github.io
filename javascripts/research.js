...
$(document).ready(function () {
	...
	$.getJSON("/json/research.json", function (data) {
		for (var i = 0; i < data.research.length; i++) {
			...
			if (documents[i].year == "2025") {
				$("#documents2025").append(documents[i].doc);
			}
			if (documents[i].year == "2024") {
				$("#documents2024").append(documents[i].doc);
			}
			if (documents[i].year == "2023") {
				$("#documents2023").append(documents[i].doc);
			}
	...
	function createResearchLong(d) {
		...
		img.setAttribute(
			"style",
			'background-image:url("/images/documents/thumbnails/' + d.img + '")'
		);
